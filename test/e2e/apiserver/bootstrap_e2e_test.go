...
//   - on first start the manifests embedded in the binary seed the built-in default
//     namespace and role (no external files / container COPY needed);
//   - the reconcile is full-overwrite, so a permission an admin adds to the built-in
//     default role via the API is reset back to the embedded list on the next start.
func TestE2E_APIServer_BootstrapReconcile(t *testing.T) {
	...
	require.NoError(t, err, "default namespace must be seeded from the embedded manifests")
	...
	assert.NotContains(t, reconciled.Spec.Permissions, "certificate:GET",
		"full-overwrite reconcile must reset admin-added permissions to the embedded list")
	assert.Contains(t, reconciled.Spec.Permissions, "agent:GET",
		"embedded permissions must remain after reconcile")
}
