...
func (m *MockServerPersistencePort) ListServers(ctx context.Context) ([]*agentmodel.Server, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1) //nolint:wrapcheck // mock error
	}
...
func (m *MockServerPersistencePort) PutServer(ctx context.Context, server *agentmodel.Server) error {
...
func TestServerService_GetServer_CacheUpdate(t *testing.T) {
	...
	oldServer := &agentmodel.Server{
		ID:              serverID,
		LastHeartbeatAt: now.Add(-30 * time.Second),
	}

	newServer := &agentmodel.Server{
		ID:              serverID,
