...
func TestNewAgent(t *testing.T) {
	...
	t.Run("Create agent with default values", func(t *testing.T) {
		...
		a := agentmodel.NewAgent(instanceUID)
	...
	t.Run("Create agent with description option", func(t *testing.T) {
		...
		a := agentmodel.NewAgent(instanceUID, agentmodel.WithDescription(description))
	...
	t.Run("Create agent with capabilities option", func(t *testing.T) {
		...
		a := agentmodel.NewAgent(instanceUID, agentmodel.WithCapabilities(&capabilities))
	...
	t.Run("Create agent with multiple options", func(t *testing.T) {
		...
		customCaps := &agentmodel.AgentCustomCapabilities{
			Capabilities: []string{"custom1", "custom2"},
		}

		a := agentmodel.NewAgent(
			instanceUID,
			agentmodel.WithDescription(description),
			agentmodel.WithCapabilities(&capabilities),
			agentmodel.WithCustomCapabilities(customCaps),
		)
...
func TestAgent_UpdateLastCommunicationInfo(t *testing.T) {
	...
	t.Run("Update last communication info with connection", func(t *testing.T) {
		...
		connection := agentmodel.NewConnection("conn-id", agentmodel.ConnectionTypeWebSocket)
		now := time.Now()

		a.UpdateLastCommunicationInfo(now, connection)
		assert.Equal(t, now, a.Status.LastReportedAt)
		assert.Equal(t, agentmodel.ConnectionTypeWebSocket, a.Status.ConnectionType)
	})
	...
	t.Run("Update last communication info without connection", func(t *testing.T) {
		...
		assert.Equal(t, now, a.Status.LastReportedAt)
		assert.Equal(t, agentmodel.ConnectionTypeUnknown, a.Status.ConnectionType)
	})
}
...
func TestAgent_RecordLastReported(t *testing.T) {
	...
	t.Run("Record last reported updates existing values", func(t *testing.T) {
		...
		a := agentmodel.NewAgent(uuid.New())
		server1 := &agentmodel.Server{
			ID: "server-1",
		}
		server2 := &agentmodel.Server{
			ID: "server-2",
		}
		time1 := time.Now()
...
func TestAgentConditions(t *testing.T) {
	...
	t.Run("New agent should have registered condition", func(t *testing.T) {
		...
		condition := agent.GetCondition(agentmodel.AgentConditionTypeRegistered)
		assert.NotNil(t, condition)
		assert.Equal(t, agentmodel.AgentConditionTypeRegistered, condition.Type)
		...
		assert.True(t, agent.IsConditionTrue(agentmodel.AgentConditionTypeRegistered))
	})
	...
	t.Run("Mark agent as connected", func(t *testing.T) {
		...
		assert.True(t, agent.IsConditionTrue(agentmodel.AgentConditionTypeConnected))

		condition := agent.GetCondition(agentmodel.AgentConditionTypeConnected)
		assert.NotNil(t, condition)
		assert.Equal(t, agentmodel.AgentConditionTypeConnected, condition.Type)
		assert.Equal(t, agentmodel.AgentConditionStatusTrue, condition.Status)
	...
	t.Run("Mark agent as disconnected", func(t *testing.T) {
		...
		assert.False(t, agent.IsConditionTrue(agentmodel.AgentConditionTypeConnected))

		condition := agent.GetCondition(agentmodel.AgentConditionTypeConnected)
		assert.NotNil(t, condition)
		assert.Equal(t, agentmodel.AgentConditionTypeConnected, condition.Type)
		assert.Equal(t, agentmodel.AgentConditionStatusFalse, condition.Status)
		assert.Equal(t, triggeredBy, condition.Reason)
	...
	t.Run("Get non-existent condition", func(t *testing.T) {
		...
		condition := agent.GetCondition(agentmodel.AgentConditionTypeHealthy)
