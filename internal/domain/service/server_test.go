...
func TestServerService_GetServer_InvalidCachedType(t *testing.T) {
	t.Parallel()
	...
	mockPersistence := new(MockServerPersistencePort)
	mockPersistence.On("GetServer", ctx, serverID).Return(mockServer, nil)
	...
	server, err := svc.GetServer(ctx, serverID)
	require.NoError(t, err)
