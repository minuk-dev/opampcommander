...
// agentPackageByNamePath is the per-item route, repeated by the read, update and delete
// entries below so a rename happens in one place.
const agentPackageByNamePath = "/api/v1/namespaces/:namespace/agentpackages/:name"
...
// @Param fieldSelector query string false "Fields: metadata.namespace, spec.packageType, spec.version"
...
func (c *Controller) List(ctx *gin.Context) {
	...
	response, err := c.agentpackageUsecase.ListAgentPackages(ctx.Request.Context(), namespace, &port.ListOptions{
