...
// certificateByNamePath is the per-item route, repeated by the read, update and delete
// entries below so a rename happens in one place.
const certificateByNamePath = "/api/v1/namespaces/:namespace/certificates/:name"

// Controller is a struct that implements the certificate controller.
type Controller struct {
	logger *slog.Logger

	certificateUsecase usecase.CertificateManageUsecase
}

// NewController creates a new instance of Controller.
func NewController(
	usecase usecase.CertificateManageUsecase,
	logger *slog.Logger,
) *Controller {
