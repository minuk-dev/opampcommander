...
func gapRouter(t *testing.T) *gin.Engine {
	...
	controller := certificate.NewController(usecasemock.NewMockUsecase(t), ctrlBase.Logger)
...
const gapBase = "/api/v1/namespaces/default/certificates"

func TestCertificateController_ListValidation(t *testing.T) {
	t.Parallel()
...
func TestCertificateController_GetValidation(t *testing.T) {
	t.Parallel()

	recorder := gapGET(t, gapRouter(t), gapBase+"/cert-1?includeDeleted=nope")

	require.Equal(t, http.StatusBadRequest, recorder.Code)
}

// TestCertificateController_MissingParams covers the required :namespace / :name validation
// branches across every handler, unreachable through routing since the segments are never empty.
func TestCertificateController_MissingParams(t *testing.T) {
	t.Parallel()

	controller := certificate.NewController(usecasemock.NewMockUsecase(t), testutil.NewBase(t).Logger)
