...
// endpointByNamePath is the per-item route, repeated by the read, update and delete
// entries below so a rename happens in one place.
const endpointByNamePath = "/api/v1/namespaces/:namespace/endpoints/:name"

// Controller is a struct that implements the endpoint controller.
type Controller struct {
	logger *slog.Logger

	endpointUsecase usecase.EndpointManageUsecase
}

// NewController creates a new instance of Controller.
func NewController(
	usecase usecase.EndpointManageUsecase,
	logger *slog.Logger,
) *Controller {
...
// @Param fieldSelector query string false "Field selector over the supported fields: metadata.namespace, spec.protocol"
