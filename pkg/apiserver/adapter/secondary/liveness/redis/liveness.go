...
func (s *Store) GetMany(
	...
	pipe := s.client.Pipeline()
	commands := make([]*goredis.MapStringStringCmd, len(instanceUIDs))

	for i, instanceUID := range instanceUIDs {
		commands[i] = pipe.HGetAll(ctx, s.recordKey(instanceUID))
	}
...
func encodeObservation(liveness *agentmodel.AgentLiveness) []any {
	return []any{
		...
		fieldConnectionType, int(liveness.ConnectionType),
...
func decodeRecord(instanceUID uuid.UUID, fields map[string]string) *agentmodel.AgentLiveness {
	...
	return &agentmodel.AgentLiveness{
		...
		ConnectionType:    agentmodel.ConnectionType(parseInt(fields[fieldConnectionType])),
		SequenceNum:       uint64(parseInt(fields[fieldSequenceNum])),
