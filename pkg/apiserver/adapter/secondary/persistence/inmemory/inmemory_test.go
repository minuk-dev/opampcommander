...
// A continue token comes from the client, so a malformed one is bad input rather
// than a server fault — the wrapping is what makes the API answer 400 instead of
// 500, and both adapters have to agree on it.
func TestStore_MalformedContinueTokenIsBadInput(t *testing.T) {
	t.Parallel()
