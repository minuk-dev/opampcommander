...
// The selector grammars are comma-separated conjunctions, so repeating the flag
// has to AND the occurrences. The map-valued -l this replaced merged repeated
// occurrences, and a plain string flag would keep only the last one — silently
// widening the result set for an invocation that already worked.
func TestListOptions_RepeatedSelectorFlagsAreAnded(t *testing.T) {
	t.Parallel()

	flags := newFlags(t,
		"-l", "service.name=otel-collector",
		"-l", "service.namespace=prod",
		"--field-selector", "status.connected=true",
		"--field-selector", "status.healthy=true",
	)

	labels, err := flags.LabelSelector()
	require.NoError(t, err)
	assert.Equal(t, "service.name=otel-collector,service.namespace=prod", labels.String())

	assert.True(t, flags.ConstrainsField("status.connected"))
	assert.True(t, flags.ConstrainsField("status.healthy"))
}

// A single occurrence must be untouched — no stray separator.
func TestListOptions_SingleSelectorFlagIsVerbatim(t *testing.T) {
	t.Parallel()

	labels, err := newFlags(t, "-l", "env=prod,!deprecated").LabelSelector()
	require.NoError(t, err)
	assert.Equal(t, "env=prod,!deprecated", labels.String())
}
