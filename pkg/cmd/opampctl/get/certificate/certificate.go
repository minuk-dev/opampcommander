...
func NewCommand(options CommandOptions) *cobra.Command {
	...
	cmd.Flags().BoolVar(&options.includeDeleted, "include-deleted", false, "Include soft-deleted certificates")
...
func (opt *CommandOptions) List(cmd *cobra.Command) error {
	...
	if opt.allNamespaces {
		certificates, err = opt.listAllNamespaces(cmd, listOpts...)
	} else {
		certificates, err = clientutil.ListCertificateFully(cmd.Context(), opt.client, opt.namespace, listOpts...)
	}
...
func (opt *CommandOptions) listAllNamespaces(
	...
	certificates, err := clientutil.ListAcrossNamespaces(
		...
		func(ctx context.Context, namespace string) ([]v1.Certificate, error) {
			return clientutil.ListCertificateFully(ctx, opt.client, namespace, listOpts...)
		},
	)
	if err != nil {
