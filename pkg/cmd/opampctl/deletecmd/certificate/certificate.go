...
var (
	// ErrCertificateNameRequired is returned when the target certificate name is not provided.
	ErrCertificateNameRequired = errors.New("certificate name is required")
)

// CommandOptions contains the options for the 'opampctl delete certificate' command.
...
// NewCommand creates a new 'opampctl delete certificate' command.
func NewCommand(options CommandOptions) *cobra.Command {
	//exhaustruct:ignore
	cmd := &cobra.Command{
		Use:               "certificate",
		Short:             "Delete certificate(s)",
...
func (o *CommandOptions) Prepare(_ *cobra.Command, args []string) error {
	if len(args) < 1 {
		return ErrCertificateNameRequired
	}
...
func (o *CommandOptions) Run(cmd *cobra.Command, names []string) error {
	...
	results := lo.Map(names, func(name string, _ int) deleteResult {
		return deleteResult{
			name: name,
			err:  o.client.CertificateService.DeleteCertificate(cmd.Context(), o.namespace, name),
		}
	})
	...
	cmd.Printf("Successfully deleted %d certificate(s): %s\n",
		len(successfullyDeleted), strings.Join(successfullyDeleted, ", "))

	if len(failedToDelete) > 0 {
		cmd.PrintErrf("Failed to delete %d certificate(s): %s\n", len(failedToDelete), strings.Join(failedToDelete, ", "))
	}

	return nil
}

// ValidArgsFunction provides dynamic completion for certificate names.
func (o *CommandOptions) ValidArgsFunction(
	...
	certificates, err := clientutil.ListCertificateFully(cmd.Context(), client, o.namespace)
	if err != nil {
		return nil, cobra.ShellCompDirectiveError
	}

	matched := lo.Filter(certificates, func(cert v1.Certificate, _ int) bool {
		return strings.Contains(strings.ToLower(cert.Metadata.Name), strings.ToLower(toComplete))
	})

	names := lo.Map(matched, func(cert v1.Certificate, _ int) string {
