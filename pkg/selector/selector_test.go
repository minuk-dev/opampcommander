...
// The same key in both maps, with different values, is the only case that
// separates "either map satisfies the requirement" from "merge the maps, one of
// them wins". Everything else agrees under both rules, so this is what pins the
// semantics the union is built on — and what a refactor to a merged map would
// have to break.
func TestMatchesAny_KeyPresentInBothMaps(t *testing.T) {
	t.Parallel()

	identifying := map[string]string{"service.version": "1.0"}
	nonIdentifying := map[string]string{"service.version": "2.0"}

	tests := map[string]struct {
		expression string
		want       bool
	}{
		"the first map's value satisfies it":               {"service.version=1.0", true},
		"the second map's value satisfies it too":          {"service.version=2.0", true},
		"a value neither map holds does not":               {"service.version=3.0", false},
		"negation fails when either map holds it":          {"service.version!=2.0", false},
		"negation holds when neither map holds it":         {"service.version!=3.0", true},
		"in is satisfied by whichever map matches":         {"service.version in (2.0,9.9)", true},
		"notin fails when either map holds a listed value": {"service.version notin (2.0)", false},
	}
