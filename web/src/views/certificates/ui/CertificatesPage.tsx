    ...
    <ResourceListPage<Certificate>
      ...
      columns={[
        { header: 'Name', render: (c) => c.metadata.name },
        {
          header: 'Has cert',
          render: (c) => (
            <Badge variant={c.spec.cert ? 'success' : 'muted'}>{c.spec.cert ? 'yes' : 'no'}</Badge>
          ),
        },
        {
          header: 'Has private key',
          render: (c) => (
            <Badge variant={c.spec.privateKey ? 'success' : 'muted'}>
              {c.spec.privateKey ? 'yes' : 'no'}
            </Badge>
          ),
        },
        {
          header: 'Has CA',
          render: (c) => (
            <Badge variant={c.spec.caCert ? 'success' : 'muted'}>
              {c.spec.caCert ? 'yes' : 'no'}
            </Badge>
          ),
        },
