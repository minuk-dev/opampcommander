...
// value missing from here would silently be unfilterable — and "which hosts did
// we fail to classify?" is exactly what `unknown` is for.
const PLATFORMS = ['kubernetes', 'docker', 'vm', 'baremetal', 'ecs', 'unknown'] as const;

// Each Platform label gets its own badge tone so the deployment environment is
// visually scannable across the host/container tabs. Anything not listed falls
// back to muted.
const PLATFORM_VARIANTS: Partial<
  Record<(typeof PLATFORMS)[number], NonNullable<VariantProps<typeof badgeVariants>['variant']>>
> = {
  kubernetes: 'primary',
  docker: 'primary',
  vm: 'success',
