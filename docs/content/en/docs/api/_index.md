...
GET /api/v1/namespaces/prod/endpoints?name=tempo-&nameContains=traces
```

`name` is a **case-sensitive prefix**, served by an index range scan. `nameContains`
is a **case-insensitive substring**. Both are matched literally — neither is ever
interpreted as a pattern — and they combine.

**On a large collection, always pair `nameContains` with `name`.** No ordered index
can answer "contains", so `nameContains` alone is matched against every document in
the collection. The page limit does not bound that: a listing reports an exact
`remainingItemCount`, which means counting the whole matched set, so the scan runs in
full on every request rather than stopping once a page is filled. Adding `name`
confines it to that prefix's indexed range.

Use `name` on its own wherever a prefix will do.
